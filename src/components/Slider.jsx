...
export default function Slider(){
    ...
    // Saving the movies data once the Slider is mounted
    useEffect(()=>{
        getTrendingMovies()
    },[])
            ...
            <div className="flex overflow-x-auto w-full px-16 py-4 scrollbar-none scroll-smooth" ref={elementRef}>
                {
                    movieList.map((item,index)=>(
                        <img src={IMAGE_BASE_URL+item.backdrop_path} alt="Movie Image" className="min-w-full md:h-[310px] object-cover object-left-top mr-5 rounded-md hover:border-[4px] border-grey-400 transition-all duration-100 ease-in" />
                    ))
                }
            </div>
        </div>
